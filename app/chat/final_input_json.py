SCORING_PROMPT_TEMPLATE = """당신은 쇼핑 심리 전문가입니다.
상품의 마케팅 키워드가 특정 유저 유형에게 얼마나 강한 충동 자극을 주는지 평가합니다.

## 입력 정보

### 마케팅 키워드 (상품명에서 추출된 심리 자극 요소)
{marketing_keywords}

### 키워드 없이 산정된 베이스 충동 점수 (0~100)
{base_score}
...
1. 각 마케팅 키워드가 유저의 F-BTI 유형과 얼마나 공명하는지 분석한다.
2. 공명도가 높을수록 base_score에서 올리고, 낮을수록 내린다.

### 축별 조정 원칙
- **U (타인 검증형)**: 판매량·랭킹·연예인 착용·리뷰 언급 키워드 → 강하게 올림 (+10~20)
- **I (자기 판단형)**: 동일 키워드 → 내림 (−5~15). 단 MADE·품질보증·사장픽 계열은 올림
- **T (트렌드 동조형)**: 유행·이번 시즌·베스트셀러 계열 → 올림 (+5~15)
- **M (차별화 추구형)**: 판매량·인기·대중적 키워드 → 내림 (−10~20). REORDER·품절임박·희소성 계열은 올림
- **E (거래 이득형)**: 할인·1+1·증정·품절임박 키워드 → 올림 (+5~15)
- **O (가치 중심형)**: 순수 할인 키워드만 있으면 무반응. 품질·MADE 보증 계열은 올림
- **D (도파민형)**: 모든 강한 키워드에 전반적으로 올림 (+5~10)
- **N (필요 기반형)**: 순수 마케팅 hype 키워드는 효과 반감 (−5~10)

### 점수 범위 규칙
- 최종 personalized_score는 0~100 범위 내
- base_score에서 최대 ±25점까지만 조정 (극단적 이탈 금지)
- 키워드가 유저 유형과 완전히 무관하면 base_score 그대로 반환
