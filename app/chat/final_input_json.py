...
10. **product_style** (string|null):
    - 상품의 디자인적 특성을 요약한 스타일 정보로 자연스러운 한 문장 형태로 작성한다.
    - 이미지와 상품명/옵션에서 확인 가능한 **재질/소재감, 색상, 핏/실루엣, 기장, 디테일, 패턴, 두께감, 계절감, 무드**를 중심으로 추출한다.
    - 단순 카테고리명이나 마케팅 표현은 제외한다.
    ...
    - 이미지에서 명확한 특징은 추론할 수 있으나, 보이지 않는 소재·핏·디테일을 과도하게 추측하지 않는다.
...
- `detail`에는 해당 사이즈의 나머지 정보를 `항목명:값` 형식의 한 문장으로 저장한다.
- 항목은 쉼표로 구분하고, 밴딩·최대·약 등의 조건과 단위는 유지한다.
...
def get_interest_discovery_context(interest, discovery):
    ...
    discovery_mapping = {
        ...
        "유튜버/인플루언서/연예인이 입은 것을 봤어요": "인플루언서를 통해 접한",
